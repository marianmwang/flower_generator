...
function drawFlower1(petalColor, petalShape, ringShape) {
  ...
  for (var x = 0; x < nring; x++) {
    var ang = x * (360 / nring);
    var cx =
      xoff + 20 * Math.cos((ang * Math.PI) / 180) + (1 * Math.random() - 0.5);
    var cy =
      yoff + 20 * Math.sin((ang * Math.PI) / 180) + (1 * Math.random() - 0.5);
...
function download_img(petalColor, petalShape, ringShape, circleColor) {
  ...
  link.download =
    "flower1_" + petalColor + petalShape + ringShape + circleColor + ".png";
  link.click();
}
