...
function drawFlower1(petalColor,petalShape,circleShape) {
    ...
    for (var x=0;x<nspetals;x++){
        var ang = x*(360/nspetals)+360/(nspetals*2)+(6*Math.random()-3);
    ...
    for (var x=0;x<nring;x++){
        var ang = x*(360/nring);
        var cx = xoff+15*Math.cos(ang*Math.PI/180)+(1*Math.random()-0.5);
        var cy = yoff+15*Math.sin(ang*Math.PI/180)+(1*Math.random()-0.5);
    ...
    for (var x=0;x<100;x++){
        var ang = x*(360/60);
        var cx = xoff+circleSize*Math.cos(ang*Math.PI/180)+(1*Math.random()-0.5);
        var cy = yoff+circleSize*Math.sin(ang*Math.PI/180)+(1*Math.random()-0.5);
...
function download_img(petalColor,petalShape,circleShape){
    ...
    link.download = 'flower1_c'+petalColor+"s"+petalShape+circleShape+'.png';
    link.click();
}
